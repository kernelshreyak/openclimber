...
func pose_for_ground(forward_input: float, turn_input: float, time: float, on_floor: bool) -> void:
	...
	_set_joint("LeftArmElbow", Vector3(-0.76, 1.1, -swing * 0.35))
	_set_joint("LeftArmWrist", Vector3(-0.64, 0.76, -swing * 0.65))
	_set_joint("LeftArmHand", Vector3(-0.58, 0.52, -swing * 0.72))
	_set_joint("RightArmShoulder", Vector3(0.54, 1.5, 0.0))
	_set_joint("RightArmElbow", Vector3(0.76, 1.1, swing * 0.35))
	_set_joint("RightArmWrist", Vector3(0.64, 0.76, swing * 0.65))
	_set_joint("RightArmHand", Vector3(0.58, 0.52, swing * 0.72))
	_set_joint("LeftLegHip", Vector3(-0.22, 0.8, 0.0))
	_set_joint("LeftLegKnee", Vector3(-0.22, 0.34, -0.02 + swing * 0.1))
	_set_joint("LeftLegAnkle", Vector3(-0.22, 0.04, -0.08 + swing * 0.07))
	_set_joint("LeftLegFoot", Vector3(-0.22, -0.1, -0.18 + swing * 0.06))
	_set_joint("RightLegHip", Vector3(0.22, 0.8, 0.0))
	_set_joint("RightLegKnee", Vector3(0.22, 0.34, -0.02 - swing * 0.1))
	_set_joint("RightLegAnkle", Vector3(0.22, 0.04, -0.08 - swing * 0.07))
	_set_joint("RightLegFoot", Vector3(0.22, -0.1, -0.18 - swing * 0.06))
...
func pose_for_climb(surface_normal: Vector3, input_vec: Vector2, time: float) -> void:
	...
	_set_joint("LeftArmElbow", Vector3(-0.82 + horizontal_shift, 1.18 + vertical_shift, -0.3))
	_set_joint("LeftArmWrist", Vector3(-0.72 + horizontal_shift, 0.92 + vertical_shift, -0.62))
	_set_joint("LeftArmHand", Vector3(-0.66 + horizontal_shift, 0.74 + vertical_shift, -0.88))
	_set_joint("RightArmShoulder", Vector3(0.54, 1.48, 0.08))
	_set_joint("RightArmElbow", Vector3(0.82 + horizontal_shift, 1.18 - vertical_shift, -0.3))
	_set_joint("RightArmWrist", Vector3(0.72 + horizontal_shift, 0.92 - vertical_shift, -0.62))
	_set_joint("RightArmHand", Vector3(0.66 + horizontal_shift, 0.74 - vertical_shift, -0.88))
	_set_joint("LeftLegHip", Vector3(-0.28, 0.82, 0.08))
	_set_joint("LeftLegKnee", Vector3(-0.4 - horizontal_shift * 0.38, 0.54 - vertical_shift * 0.6, -0.12))
	_set_joint("LeftLegAnkle", Vector3(-0.38 - horizontal_shift * 0.3, 0.24 - vertical_shift, -0.46))
	_set_joint("LeftLegFoot", Vector3(-0.34 - horizontal_shift * 0.25, 0.04 - vertical_shift, -0.74))
	_set_joint("RightLegHip", Vector3(0.28, 0.82, 0.08))
	_set_joint("RightLegKnee", Vector3(0.4 - horizontal_shift * 0.38, 0.54 + vertical_shift * 0.6, -0.12))
	_set_joint("RightLegAnkle", Vector3(0.38 - horizontal_shift * 0.3, 0.24 + vertical_shift, -0.46))
	_set_joint("RightLegFoot", Vector3(0.34 - horizontal_shift * 0.25, 0.04 + vertical_shift, -0.74))
